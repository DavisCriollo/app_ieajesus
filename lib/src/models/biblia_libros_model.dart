...
// To parse this JSON data, do
//
//     final bibliaModel = bibliaModelFromMap(jsonString);
...
class BibliaModel {
    ...
    factory BibliaModel.fromJson(String str) => BibliaModel.fromMap(json.decode(str));

    String toJson() => json.encode(toMap());

    factory BibliaModel.fromMap(Map<String, dynamic> json) => BibliaModel(
        biblia: Map.from(json["biblia"]).map((k, v) => MapEntry<String, List<List<String>>>(k, List<List<String>>.from(v.map((x) => List<String>.from(x.map((x) => x)))))),
    );
